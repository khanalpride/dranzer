<template>
  <scaffolding-component-container
    :heading="`Many To Many Relations (${relations.length})`"
    :loading="loading || fetchingMutations">
    <row>
      <column :size="hasSelection ? 12 : 8" :offset="hasSelection ? 0 : 2">
        <pg-input v-model="query" placeholder="Search for possible many-to-many relations..."/>
      </column>
      <column :size="hasSelection ? 12 : 8" :offset="hasSelection ? 0 : 2" push5>
        <p class="small text-info hint-text">e.g. Project accesses many Deployments and vice versa</p>
      </column>
    </row>
    <row v-if="groups.length">
      <column :size="hasSelection ? 12 : 8" :offset="hasSelection ? 0 : 2" push5>
        <p class="small text-primary">
          Showing <strong>{{ filteredGroups.length }}</strong> / <strong>{{ groups.length }}</strong> available
        ...
        <row>
          ...
          <column :key="rel.source.id + rel.related.id + rel.type" v-for="rel in group.relations">
            <pg-check-box no-margin
              ...
              <template slot="label">
                <strong>{{ rel.source.name }}</strong> <span class="text-complete bold">accesses many</span> <strong>{{
                  str.pluralize(rel.related.name)
                }} <span class="text-complete">and vice versa</span> <span class="text-primary">through the intermediate table</span></strong>
              </template>
            </pg-check-box>
          </column>
            ...
            <row>
              <column>
                ...
                <pg-input v-model="group.intermediateTable"
                          ...
                          class="m-t-5" @input="persist(group)" />
              </column>
              <column>
              ...
              <column push10 v-if="group.showColumns">
                <button class="btn btn-primary" :disabled="group.columns.length > 4" @click="addCustomColumn({}, group)">
...
export default {
  name: 'ManyToManyRegularRelationsContainer',
  ...
  computed: {
    filteredGroups() {
      ...
      return this.groups.filter((g) => g.relations.find((rel) => {
        const normalCase = `${rel.source.name} accesses many ${rel.related.name}`.toLowerCase();
        const inverseCase = `${rel.related.name} accesses many ${rel.source.name}`.toLowerCase();
  ...
  methods: {
    async syncRelations() {
      const { data } = await this.mutation({
        path: 'eloquent/relations/regular/many-to-many',
        like: true,
        refresh: true,
      });
        ...
        otherModels.forEach((otherModel) => {
          ...
          if (otherModelColumn) {
            const group = groups.map((g) => g.relations).flat().find(
              ...
              || (r.source.id === otherModel.id && r.related.id === modelId),
            );
            ...
            const persisted = this.relations.find(
              (r) => (r.source.id === modelId && r.related.id === otherModel.id)
              || (r.source.id === otherModel.id && r.related.id === modelId),
            ) || {};
            ...
            const modelForeignKeyColumnPrefix = `${this.str.snakeCase(modelName).toLowerCase()}`;
            const otherModelForeignKeyColumnPrefix = `${this.str.snakeCase(otherModel.modelName).toLowerCase()}`;

            const columns = persistedColumns.length ? [] : [{
              id: Math.round(Math.random() * Number.MAX_SAFE_INTEGER),
              name: `${modelForeignKeyColumnPrefix}_id`,
              ...
              id: Math.round(Math.random() * Number.MAX_SAFE_INTEGER),
              name: `${otherModelForeignKeyColumnPrefix}_id`,
              type: 'unsignedBigInteger',
              attributes: {
                ai: false,
                us: true,
            ...
            const intermediateTableName = `${otherModelForeignKeyColumnPrefix}_${modelForeignKeyColumnPrefix}`;

            groups.push({
              id: persisted.id || Math.round(Math.random() * Number.MAX_SAFE_INTEGER),
              showColumns: !persisted,
              intermediateTable: persisted.intermediateTable || intermediateTableName,
              columns: finalColumns,
              relations: [
                {
                  enabled: persisted.id !== undefined,
                  source: {
                    id: modelId,
                    name: modelName,
                  },
    ...
    persist(group) {
      ...
      this.$nextTick(() => {
        ...
        const name = 'Many To Many Relation';
        const path = `eloquent/relations/regular/many-to-many/${sourceModel.id}/${relatedModel.id}`;
    ...
    onGroupedRelationToggled(checked, relation, group) {
      if (checked) {
        ...
        const rIndex = this.relations.findIndex((r) => r.source.id === relation.source.id);
        if (rIndex > -1) {
          this.deleteMutation(`eloquent/relations/regular/many-to-many/${relation.source.id}/${relation.related.id}`, {
            then: () => {
