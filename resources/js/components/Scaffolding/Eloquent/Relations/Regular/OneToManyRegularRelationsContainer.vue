<template>
  <scaffolding-component-container
    :heading="`One To Many Relations (${relations.length})`"
    ...
    <div v-else>
      <row>
        <column size="8" offset="2">
          <pg-input v-model="query" placeholder="Search for possible one-to-many relations"/>
        </column>
        <column push5 size="8" offset="2">
          <p class="small text-info hint-text">e.g. User has many Products</p>
        </column>
      </row>
      <row>
        <column push5 size="8"
          ...
          <row>
            ...
            <column :key="rel.source.id + rel.related.id + rel.type" v-for="rel in group.relations">
              <pg-check-box no-margin
                            ...
                            v-if="rel.type === 'hasMany'">
                <template slot="label">
                  <strong>{{ rel.source.name }}</strong> <span class="text-complete bold">has many</span> <strong>{{
...
export default {
  name: 'OneToManyRegularRelationsContainer',
  ...
  computed: {
    filteredGroups() {
      ...
      return this.groups.filter((g) => g.relations.find((rel) => {
        const hasManyStr = `${rel.source.name} has many ${rel.related.name}`.toLowerCase();
        const belongsToStr = `${rel.source.name} belongs to ${rel.related.name}`.toLowerCase();

        try {
          return (new RegExp(query).test(hasManyStr)) || (new RegExp(query).test(belongsToStr));
        } catch (e) {
          return null;
        }
      }));
  ...
  methods: {
    async syncRelations() {
      const { data } = await this.mutation({
        path: 'eloquent/relations/regular/one-to-many',
        like: true,
        refresh: true,
      });
    ...
    getGroups() {
      ...
      models.forEach((m) => {
        ...
        otherModels.forEach((otherModel) => {
          ...
          if (otherModelColumn) {
            const hasManyRel = this.relations.find((r) => r.source.id === modelId && r.related.id === otherModel.id && r.type === 'hasMany');
            const belongsToRel = this.relations.find((r) => r.source.id === otherModel.id && r.related.id === modelId && r.type === 'belongsTo');
            groups.push({
              id: Math.round(Math.random() * Number.MAX_SAFE_INTEGER),
              relations: [
                {
                  enabled: hasManyRel || false,
                  source: {
                    id: modelId,
                    name: modelName,
                  },
    ...
    persist(relation) {
      this.$nextTick(() => {
        const sourceModel = relation.source.id;
        const relatedModel = relation.related.id;

        const name = 'One To Many Relation';
        const path = `eloquent/relations/regular/one-to-many/${sourceModel}/${relatedModel}`;
    ...
    onGroupedRelationToggled(checked, relation) {
      if (!checked) {
        ...
        if (relationIndex > -1) {
          this.deleteMutation(`eloquent/relations/regular/one-to-many/${relation.source.id}/${relation.related.id}`, {
    ...
    onToggleCheckAll(checkAll) {
      ...
      this.groups.map((g) => g.relations).flat().forEach((relation) => {
        ...
        allMutations.push({
          name: `One To Many Relation (${relation.source.id} & ${relation.related.id})`,
          path: `eloquent/relations/regular/one-to-many/${relation.source.id}/${relation.related.id}`,
          value: {
            id: Math.round(Math.random() * Number.MAX_SAFE_INTEGER),
            source: relation.source,
            related: relation.related,
