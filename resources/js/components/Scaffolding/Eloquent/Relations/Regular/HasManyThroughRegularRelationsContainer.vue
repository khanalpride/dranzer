<template>
  <scaffolding-component-container
    :heading="`Has Many Through Relations (${relations.length})`"
    :loading="loading || fetchingMutations">
    <row>
      <column :size="hasSelection ? 12 : 8" :offset="hasSelection ? 0 : 2">
        <pg-input v-model="query" placeholder="Search for possible has-many-through relations..."/>
      </column>
      <column :size="hasSelection ? 12 : 8" :offset="hasSelection ? 0 : 2" push5>
        <p class="small text-info hint-text">e.g. Project accesses many Deployments</p>
      </column>
    </row>
...
export default {
  name: 'HasManyThroughRegularRelationsContainer',
  ...
  methods: {
    async syncRelations() {
      const { data } = await this.mutation({
        path: 'eloquent/relations/regular/has-many-through',
        like: true,
        refresh: true,
      });
              ...
              relations: [
                {
                  ...
                  enabled: firstCaseRel !== undefined || false,
                  source: {
                    id: modelId,
                    name: modelName,
                  },
    ...
    persist(group) {
      ...
      this.$nextTick(() => {
        ...
        const name = 'Has Many Through Relation';
        const path = `eloquent/relations/regular/has-many-through/${sourceModel.id}/${relatedModel.id}`;
    ...
    onGroupedRelationToggled(checked, relation, group) {
      if (checked) {
        ...
        if (secondCaseRel) {
          this.deleteMutation(`eloquent/relations/regular/has-many-through/${relation.related.id}/${relation.source.id}`, {
            then: () => {
        ...
        if (rIndex > -1) {
          this.deleteMutation(`eloquent/relations/regular/has-many-through/${relation.source.id}/${relation.related.id}`, {
            then: () => {
              this.relations.splice(rIndex, 1);
